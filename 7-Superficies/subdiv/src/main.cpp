...
int current_model = 0;
bool fillx = false, nodes = true, wireframe = true, smooth = false, 
...
void subdivide(SubDivMesh &mesh) {
	...
	Mapa m_mapa;
	int n_n = mesh.n.size(); // cant. puntos originales
	int n_e = mesh.e.size(); // cant. elementos originales
	///PASO 1:
	// 1) Agregar los centroides:
	for(int i=0; i<n_e; i++)  { // por cada elemento
		Elemento e = mesh.e[i];
		glm::vec3 suma = glm::vec3(0.f,0.f,0.f);
		for(int j=0; j<e.nv; j++){ //e.nv retorna num. vertices de un elemento(3 o 4)
			Nodo n = mesh.n[e.n[j]]; 
			suma += n.p; // voy sumando los nodos (suma de puntos)
		}
		Nodo centroide = suma/float(e.nv); //divido la suma por num. vertices
		mesh.n.push_back(centroide); //guardo los centroides en el vector n
	}
	///PASO 2:
	// 2) Agregar los nodos asociados por cada arista
	for(int i=0; i<n_e; i++) { // por cada elemento recorro
		Elemento e = mesh.e[i];
		glm::vec3 nuevo;
		
		for(int j=0;j<e.nv;j++){ // por cada nodo del elemento armo una arista
			Arista arista(e[j],e[j+1]); 
			
			if (m_mapa.find(arista) == m_mapa.end()){
	...
	for(int i = 0; i < n_e; i++) { // por cada elemento original
		Elemento e = mesh.e[i];
		int centroide = n_n + i; //los centroides en el arreglo de nodos comienzan en numero nodos + 1, +2, +cant.elem.
		
		for(int j=0; j<e.nv; j++){ // con cada nodo vecino armo elementos nuevos
			int nodo0 = e[j];
			int nodo1 = m_mapa[Arista(e[j],e[j+1])]; // m_mapa me devuelve indice de nodo
			int nodo2 = m_mapa[Arista(e[j],e[j-1])];
	...
	// mapa auxiliar en los casos pertinentes (cuando el nodo no es frontera o cuando si es 
	// frontera pero el otro nodo de la arista tambien).
	...
	for(int i=0;i<n_n;i++) { 
		...
		if(nodoActual.es_frontera){
			...
			for(int j=0;j<nodoActual.e.size();j++) { 
				...
				f+=mesh.n[e[0]].p; // habiamos puesto los centroides en el [0] en el paso 3
			}
