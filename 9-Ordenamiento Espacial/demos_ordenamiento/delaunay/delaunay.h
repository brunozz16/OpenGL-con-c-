...
public:
  ...
  long aa;          // cuadruple del area => a=aa/4
  long cc[2];       // cc=aa*(c-p0) => c=p0+cc/aa
  ...
  // funciones de forma
  void fforma(const p2e &pt, long *ai) const{
  ...
  bool tiene_t(const p2e &pt, long *ai) const{ // triangulo (calcula las coords de area)
    ai[3]=aa; pt.fforma(*p[0],*p[1],*p[2],ai,true);
  ...
  bool contiene_t(const p2e &pt, long *ai) const{ // triangulo (calcula las coords de area)
    ai[3]=aa; pt.fforma(*p[0],*p[1],*p[2],ai,true);
    if (ai[0]<=0||ai[1]<=0||ai[2]<=0) return false;
    return true;
  }
...
public:
  ...
  Dtri& enquetriangulo(const p2e &p, long a[4], Dtri *start=0);
