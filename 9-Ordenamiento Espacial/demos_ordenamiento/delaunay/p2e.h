...
public:
  ...
  // comparación
  bool eq_b2(const int*,const long r2=0) const; // circulo dado r^2
  bool eq_b2(const p2e& p,const long r2=0) const {return eq_b2(p.coord,r2);}
  ...
  p2e operator-(const p2e &p) const // resta
    {return p2e(x-p.x,y-p.y);}
  long operator%(const p2e &p) const // producto vectorial (% =precedencia *)
    {return x*p.y-y*p.x;}
  long operator*(const p2e &p) const // producto escalar
    {return x*p.x+y*p.y;}
  ...
  // distancia
  long distancia2(const p2e &p) const // distancia al cuadrado
    {return cuadrado<long>(x-p.x)+cuadrado<long>(y-p.y);}
  friend long distancia2(const p2e &a,const p2e &b)
  ...
  // modulo
  long mod2() const   // modulo al cuadrado
    {return cuadrado<long>(x)+cuadrado<long>(y);}
  friend long mod2(const p2e &p) {return p.mod2();}
  ...
  bool interseccion( // la pone en *this
      const p2e &p00, const p2e &p01, 
      const p2e &p10, const p2e &p11, 
      long params[3]); // los parametros reales son 0/2 y 1/2
  ...
  friend void c3(const p2e &, const p2e &, const p2e &, long *c, long &a);
  friend void c3 (const p2e **p,long *c, long &a)
    {c3(*(p[0]),*(p[1]),*(p[2]),c,a);}
  ...
  p2e(const p2e &p0, const p2e &p1, const p2e &p2, const long *a);
  // calcula las tres coordenadas de area
  // si el area ya viene precalculada poner true
  void fforma(const p2e&, const p2e&, const p2e&, long *, bool area_calculada=false) const;
};
