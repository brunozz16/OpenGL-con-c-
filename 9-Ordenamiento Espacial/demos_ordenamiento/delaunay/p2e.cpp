...
#include "p2e.h"
  ...
  // comparación
bool p2e::eq_b2(const int* p,const long r2) const { // circulo dado r^2
  long d2=cuadrado<long>(p[0]-x); if (d2>r2) return false;
  d2+=cuadrado<long>(p[1]-y); if (d2>r2) return false;
  return true;
}
...
bool hay_interseccion_estricta(
  ...
  // c es (centro-p0)*a
void c3(const p2e &p0, const p2e &p1, const p2e &p2, long *c, long &a){
  p2e P1=p1-p0,P2=p2-p0; // origen en p0
  a=(P1%P2)<<1;
  long m2P1=P1.mod2(),m2P2=P2.mod2();
...
p2e::p2e(const p2e &p0, const p2e &p1, const p2e &p2, const long *a){// constructor
  if (!a[3]) {x=y=0; return;}
...
void p2e::fforma(const p2e &p0, const p2e &p1, const p2e &p2, long *a, bool area_calculada) const {
