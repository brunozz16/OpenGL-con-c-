...
int main() {
	...
	do {
		...
		// Acá limpiamos el stencil buffer
		glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT | GL_STENCIL_BUFFER_BIT); 
		
		// Armo los valores de stencil buffer, que van a ser 0, 1, 2
		glStencilOp(GL_KEEP, GL_KEEP, GL_REPLACE); //cuando pasa sstencil y z reemplaza, cuando pasan los dos reemplaza
		glStencilFunc(GL_ALWAYS, 1, 0xFF); // todos los fragmentos pasan siempre(1111 1111) cada bit pasa como esta
		glStencilMask(0xFF); // la mascara es la misma que arriba para escribir el stencil buffer
		drawFloor(true);
		
		glStencilOp(GL_KEEP, GL_KEEP, GL_INCR); //cuando pasa sstencil y z reemplaza
		...
		// Tengo que definir bien la pregunta
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); //cuando pasa sstencil y z reemplaza
		glStencilFunc(GL_NOTEQUAL, 0, 0xFF ); // pasa si el valor del stencil buffer es igual a (ref=2 && mask=2)
		drawObject(reflection);
		
		/// @todo: seleccionar la mascara y el valor de referencia adecuado para cada objeto
		
		//definir nuevamente la pregunta
		glStencilOp(GL_KEEP, GL_KEEP, GL_KEEP); //cuando pasa sstencil y z reemplaza
		glStencilFunc(GL_EQUAL, 1, 0xFF );
		drawFloor(true);
		//definir nuevamente la pregunta
